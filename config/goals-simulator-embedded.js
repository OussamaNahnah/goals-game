...
*/
        ...
        let draggedWaypointColor = "-";
        let draggedExclusivePointData = null;
        let draggedExclusiveColor = "-";
        ...
        const toggleMovementBtn = document.getElementById("toggleMovementBtn");
        const editorColumn = document.getElementById("editor-column");
        const movementColumn = document.getElementById("movement-column");
        const mainContainer = document.querySelector(".main-container");

        // --- Help Modal Elements ---
        const parametersHelpBtn = document.getElementById("parametersHelpBtn");
        const parametersHelpModal = document.getElementById(
          "parametersHelpModal",
        );
        const closeParametersHelpModal = document.getElementById(
          "closeParametersHelpModal",
        );
        ...
        function drawRobots(ctx, robots, frameBoundaries, rotation = 0) {
          ...
          robots.forEach(([color, x, y]) => {
            ...
            ctx.fillStyle =
              letterColorMap[color] || simConfig.colors[color] || "gray";
            ctx.beginPath();
        ...
        function drawAllColorsPie(ctx, cx, cy, radius) {
          const _activeLetters = (
            document.getElementById("color_letters_input")?.value || ""
          )
            .split(",")
            .map((s) => s.trim())
            .filter((s) => s);
          const colors = _activeLetters
            .map((l) => letterColorMap[l])
            .filter(Boolean);
          if (colors.length === 0) {
            ctx.fillStyle = "#888";
            ctx.beginPath();
        ...
        function drawWaypoints(ctx, waypoints, frameBoundaries, rotation = 0, robots = []) {
          ...
          waypoints.forEach((waypoint) => {
            ...
            // For '*' waypoints co-located with a robot, offset so both are visible
            ...
            if (char === "*") {
              ...
              bgColor =
                char && char !== "-"
                  ? letterColorMap[char] || simConfig.waypointColor
                  : "#000000";
            ...
            const pR = size * 0.45;
            const headCy = cy - size * 0.38;
            const tipY = cy + size * 0.68;
            const d = tipY - headCy;
            const sinA = pR / d;
            ...
            ctx.beginPath();
            ctx.arc(cx, headCy, pR, endAngle, startAngle, true);
            ctx.lineTo(cx, tipY);
            ctx.closePath();
            ctx.fill();
        ...
        function drawExclusivePoints(
          ...
          exclusivePoints.forEach((exclusivePoint) => {
            ...
            let char, x, y;
            if (
              exclusivePoint.length === 3 &&
              typeof exclusivePoint[0] === "string"
            ) {
            ...
            if (char === "*") {
              ...
              if (char && char !== "-") {
                ctx.fillStyle =
                  letterColorMap[char] || simConfig.exclusivePointColor;
              } else {
        ...
        function drawWalls(ctx, walls, frameBoundaries, rotation = 0) {
          ...
          walls.forEach((wall) => {
            ...
            if (wallIntersections.length === 0) {
              ...
              // Uses (0,0) as the fixed reference origin.
              //
              // Vertical wall at x=wx, clipped by horizontal wall at y=wy (intersection = (wx, wy)):
              //   wy ≤ 0 → corner is in lower half → keep UPPER arm (y > wy) → replace START
              //   wy > 0 → corner is in upper half → keep LOWER arm (y < wy) → replace END
              //
              // Horizontal wall at y=wy, clipped by vertical wall at x=wx (intersection = (wx, wy)):
              //   wx > 0 → corner is in right half → keep RIGHT arm (x > wx) → replace START
              //   wx ≤ 0 → corner is in left  half → keep LEFT  arm (x < wx) → replace END
        ...
        function closeFrame(configId) {
          // Save the vertical scroll position of simulator-column before re-rendering
          const simulatorColumn = document.getElementById("simulator-column");
          const savedScrollTop = simulatorColumn
            ? simulatorColumn.scrollTop
            : 0;
          ...
          // Restore the vertical scroll position after re-rendering
          if (simulatorColumn) {
            // Use requestAnimationFrame to ensure DOM is fully rendered
            requestAnimationFrame(() => {
              simulatorColumn.scrollTop = savedScrollTop;
            });
          }
        }
        ...
        function updateFrameTitle(configId) {
          ...
          if (config && frameElements[configId]?.titleElement) {
            ...
            // Update title text
            frameElements[configId].titleElement.textContent =
              `Simulation ${frameIndex} (Total Steps: ${totalSteps})`;
          }
        }
              ...
              if (waypoints && waypoints.length > 0) {
                config.gridWaypoints[gridIndex] = waypoints.map((waypoint) => {
                  if (waypoint.length === 2) {
                    ...
                    waypoint.length === 3 &&
                    typeof waypoint[0] === "string"
                  ) {
                    // New format [char, x, y]
                    const [char, x, y] = waypoint;
                    const [newX, newY] = rotate_point(x, y, rotationDelta);
                    return [char, newX, newY];
                  } else if (waypoint.length === 4) {
                    // Format [symbol, x, y, direction]
        ...
        function handlePaletteDragStart(e) {
          ...
          if (type === "waypoint") {
            // Waypoint drag
            const waypointCharColor = paletteElement.dataset.color || "-";
            draggedItem = "waypoint";
            draggedWaypointData = [waypointCharColor, 0, 0];
            draggedWaypointColor = waypointCharColor;
            isNewWaypointFromPalette = true;
            sourceCanvasElement = null;
            sourceConfigId = null;

            if (waypointCharColor === "*") {
              ...
              const _activeLetters = (
                ...
                .map((s) => s.trim())
                .filter((s) => s);
              const _colors = _activeLetters
                .map((l) => letterColorMap[l])
                .filter(Boolean);
              const _step = _colors.length > 0 ? 100 / _colors.length : 100;
              const _stops = _colors
                .map((c, i) => `${c} ${i * _step}% ${(i + 1) * _step}%`)
                .join(", ");
              ...
              dragGhost.style.background =
                _colors.length > 0
                  ? `${_pinSvg}, conic-gradient(${_stops})`
                  : "#888";
              dragGhost.style.backgroundColor = "";
            } else {
              const waypointBgColor =
                waypointCharColor !== "-"
                  ? letterColorMap[waypointCharColor] || simConfig.waypointColor
                  : "#000000";
              dragGhost.style.background = "";
              dragGhost.style.backgroundColor = waypointBgColor;
              dragGhost.style.backgroundImage =
                'url(\'data:image/svg+xml;utf8,<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="%23ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>\')';
            ...
            const exclusiveCharColor = paletteElement.dataset.color || "-";
            draggedItem = "exclusive_point";
            draggedExclusivePointData = [exclusiveCharColor, 0, 0];
            draggedExclusiveColor = exclusiveCharColor;
            isNewExclusivePointFromPalette = true;
            sourceCanvasElement = null;
            sourceConfigId = null;

            if (exclusiveCharColor === "*") {
              ...
              dragGhost.style.background =
                _colors.length > 0
                  ? `${_xSvg}, conic-gradient(${_stops})`
                  : "#888";
              dragGhost.style.backgroundColor = "";
            } else {
              const exclusiveBgColor =
                exclusiveCharColor !== "-"
                  ? letterColorMap[exclusiveCharColor] ||
                    simConfig.exclusivePointColor
                  : "#000000";
              dragGhost.style.background = "";
              dragGhost.style.backgroundColor = exclusiveBgColor;
              dragGhost.style.backgroundImage =
                'url(\'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="%23fff" viewBox="0 0 16 16"><path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8 2.146 2.854Z"/></svg>\')';
              dragGhost.style.backgroundSize = "contain";
              dragGhost.style.backgroundRepeat = "no-repeat";
              dragGhost.style.backgroundPosition = "center";
            }
        ...
        function handleCanvasMouseDown(e) {
          ...
          for (let i = gridWaypoints.length - 1; i >= 0; i--) {
            const wp = gridWaypoints[i];
            const wpChar = wp.length === 3 ? wp[0] : "-";
            const wX = wp.length === 3 ? wp[1] : wp[0];
            const wY = wp.length === 3 ? wp[2] : wp[1];
            const { cx: waypointCX, cy: waypointCY } = frameWorldToCanvas(wX, wY);
            // For any non-general waypoint co-located with a robot, hit-test against offset position
            ...
            if (distance <= simConfig.waypointRadius + 2) {
              ...
              if (draggedWaypointData.length === 2)
                draggedWaypointData = [
                  "-",
                  draggedWaypointData[0],
                  draggedWaypointData[1],
                ];
              ...
              const wpBgColor =
                wpChar !== "-"
                  ? letterColorMap[wpChar] || simConfig.waypointColor
                  : "#000000";
              if (wpChar === "*") {
                const _activeLetters = (
                  document.getElementById("color_letters_input")?.value || ""
                ...
                dragGhost.style.background = "";
                dragGhost.style.backgroundColor = wpBgColor;
                dragGhost.style.backgroundImage =
                  'url(\'data:image/svg+xml;utf8,<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="%23ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>\')';
                dragGhost.style.backgroundSize = "contain";
                dragGhost.style.backgroundRepeat = "no-repeat";
          ...
          for (let i = gridExclusivePoints.length - 1; i >= 0; i--) {
            const ep = gridExclusivePoints[i];
            const epChar = ep.length === 3 ? ep[0] : "-";
            const eX = ep.length === 3 ? ep[1] : ep[0];
            const eY = ep.length === 3 ? ep[2] : ep[1];
            ...
            if (distance <= simConfig.exclusivePointRadius + 2) {
              ...
              if (draggedExclusivePointData.length === 2)
                draggedExclusivePointData = [
                  "-",
                  draggedExclusivePointData[0],
                  draggedExclusivePointData[1],
                ];
              ...
              const epBgColor =
                epChar !== "-"
                  ? letterColorMap[epChar] || simConfig.exclusivePointColor
                  : "#000000";
              if (epChar === "*") {
                const _activeLetters = (
                  document.getElementById("color_letters_input")?.value || ""
                ...
                dragGhost.style.background = "";
                dragGhost.style.backgroundColor = epBgColor;
                dragGhost.style.backgroundImage =
                  'url(\'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="%23fff" viewBox="0 0 16 16"><path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8 2.146 2.854Z"/></svg>\')';
                dragGhost.style.backgroundSize = "contain";
                dragGhost.style.backgroundRepeat = "no-repeat";
              ...
              dragGhost.style.height = `${
                simConfig.exclusivePointRadius * 2
              }px`;
              dragGhost.style.borderRadius = "50%";
              dragGhost.style.transform = "translate(-50%, -50%)";
              canvas.classList.add("dragging-source-canvas");
          ...
          for (let i = currentRobots.length - 1; i >= 0; i--) {
            ...
            if (distance <= simConfig.robotRadius + 2) {
              ...
              dragGhost.style.backgroundColor =
                letterColorMap[draggedRobotData[0]] ||
                simConfig.colors[draggedRobotData[0]] ||
                "gray";
              dragGhost.style.display = "block";
              dragGhost.style.left = `${e.clientX}px`;
              dragGhost.style.top = `${e.clientY}px`;
        ...
        function handleDragEnd(e) {
          ...
          if (isFromPalette && targetCanvas) {
            ...
            if (config) {
              ...
              const isInside = isWithinBoundaries(
                dropCoords.x,
                dropCoords.y,
                boundaries,
              );
              ...
              if (
                (draggedItem === "waypoint" ||
                  draggedItem === "exclusive_point") &&
                parseInt(targetCanvas.dataset.gridIndex, 10) === 0
              ) {
                shouldAnimateReturn = true;
              }
            } else {
        ...
        function handleRobotDrop(targetCanvas, currentX, currentY) {
          ...
          const isValidDrop =
            droppedInsideValidCanvas && isDropInsideTargetBoundaries;
        ...
        function handleWaypointDrop(targetCanvas, currentX, currentY) {
          ...
          if (droppedInsideValidCanvas && isDropInsideTargetBoundaries) {
            ...
            const waypointChar =
              draggedWaypointData && draggedWaypointData.length === 3
                ? draggedWaypointData[0]
                : "-";
        ...
        function handleExclusivePointDrop(targetCanvas, currentX, currentY) {
          ...
          if (droppedInsideValidCanvas && isDropInsideTargetBoundaries) {
            ...
            // Add the new exclusive point at the position (using [char, x, y] format)
            const exclusiveChar =
              draggedExclusivePointData &&
              draggedExclusivePointData.length === 3
                ? draggedExclusivePointData[0]
                : "-";
        ...
        function updateBoundaryGhostPosition(clientX, clientY, canvas, rect) {
          ...
          switch (draggedBoundaryEdge) {
            case "xmin":
            case "xmax":
              boundaryGhost.style.left = `${
                mouseX_canvas + rect.left - boundaryWidthPx / 2
              }px`;
            ...
            case "ymax":
              ...
              boundaryGhost.style.top = `${
                mouseY_canvas + rect.top - boundaryWidthPx / 2
              }px`;
        ...
        function initializeResizeAndToggle() {
          ...
          document.addEventListener("mousemove", (e) => {
            ...
            // Only resize when a column is open
            const rightCol = movementColumn && movementColumn.style.display !== "none"
              ? movementColumn
              : (editorColumn && editorColumn.style.display !== "none" ? editorColumn : null);
            if (!rightCol) return;
            ...
            const rightWidth = 100 - simulatorWidth;
          ...
          // Toggle panel buttons — mutually exclusive panels
          function setPanelOpen(col, btn, open) {
            if (!col || !btn) return;
            const arrow = btn.querySelector('.tpb-arrow');
            if (open) {
              col.style.display = "flex";
              btn.classList.replace('is-closed', 'is-open');
              if (arrow) arrow.textContent = '▾';
            } else {
              col.style.display = "none";
              btn.classList.replace('is-open', 'is-closed');
              if (arrow) arrow.textContent = '▸';
            }
          }

          if (toggleMovementBtn && movementColumn) {
            toggleMovementBtn.addEventListener("click", function (e) {
              e.stopPropagation();
          ...
          if (toggleEditorBtn && editorColumn) {
        ...
        function toggleEditor() {
          // Legacy: used in iframe detection below — switch to editor view
          if (editorColumn && editorColumn.style.display === "none") {
            editorColumn.style.display = "flex";
            if (toggleEditorBtn) {
              toggleEditorBtn.classList.replace('is-closed', 'is-open');
              const arrow = toggleEditorBtn.querySelector('.tpb-arrow');
              if (arrow) arrow.textContent = '▾';
            }
            if (movementColumn) movementColumn.style.display = "none";
            if (toggleMovementBtn) {
              toggleMovementBtn.classList.replace('is-open', 'is-closed');
              const arrow = toggleMovementBtn.querySelector('.tpb-arrow');
              if (arrow) arrow.textContent = '▸';
            }
          }
        }
        ...
        function addNewFrame() {
          const newConfig = {
            id: `cfg-new-${Date.now()}`,
            grids: [[], []], // Start with empty grids (no robots/obstacles by default)
      ...
      */
        ...
        function setupModalAndParser() {
          ...
          if (closeModalBtn)
            closeModalBtn.addEventListener("click", () => {
              ...
              loadModal.querySelector("p").innerHTML =
                'You can paste either:<br />• JSON: <code>{"simulation_configs": [...]}</code><br />• Complete HTML simulation file';
              loadModal.querySelector("#versionSelector").style.display =
                "block";
              loadModal.querySelector("#submitConfigBtn").textContent =
                "Load Configuration";
          ...
          if (submitConfigBtn)
            submitConfigBtn.addEventListener("click", () => {
              ...
              try {
                ...
                console.error(
                  `Error parsing ${
                    isAddGoalsMode ? "goal" : selectedFormat.toUpperCase()
                  } configuration:`,
                  error,
                );
                alert(
          ...
          configBlocks.forEach((blockContent, blockIndex) => {
            try {
              ...
              targets.forEach((target) => {
                ...
                gridExclusivePoints.push(
                  normalizePoints(target.exclusive || []),
                );
              });
        ...
        function initialize() {
          ...
          if (window.self !== window.top) {
            ...
            // In iframe mode, hide the editor panel (keep master grid visible by default)
            if (editorColumn) editorColumn.style.display = "none";
            if (toggleEditorBtn) {
              toggleEditorBtn.classList.replace('is-open', 'is-closed');
              const arrow = toggleEditorBtn.querySelector('.tpb-arrow');
              if (arrow) arrow.textContent = '▸';
            }
          }
